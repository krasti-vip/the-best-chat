...
import ru.test.the.best.chat.repository.UserRepository;
...
public class UserService implements ServiceChat<UserResponse, UUID, CreateUserRequest> {
    ...
    @Override
    public Result<Optional<UserResponse>, Error> findById(UUID id) {
    ...
    @Override
    @Transactional
    public UnitResult<Error> updateById(final UUID id, final CreateUserRequest createUserRequest) {
    ...
    public Result<Optional<UserResponse>, Error> findByName(String userName) {
        log.debug("Attempting to find user by name: {}", userName);
        ...
        try {
            final Optional<UserResponse> userResponse = userRepository.findByUsername(userName)
                    .map(User::toUserResponse);
