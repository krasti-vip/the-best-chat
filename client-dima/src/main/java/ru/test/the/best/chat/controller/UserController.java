...
public class UserController {
    ...
    public ResponseEntity<ApiResultResponse<Void>> updateUser(
        ...
        var result = userService.updateById(id, request);
    ...
    @GetMapping("/{userName}")
    public ResponseEntity<ApiResultResponse<UserResponse>> getUserByName(
            @Parameter(description = "userName пользователя", required = true)
            @PathVariable String userName) {

        log.info("REST: GET /api/v1/users/{} - Fetching user by userName", userName);

        var result = userService.findByName(userName);

        if (result.isFailure()) {
            log.warn("REST: Failed to fetch user with userName: {}", userName);
            return ResponseEntity
                    .status(HttpStatus.BAD_REQUEST)
                    .body(ApiResultResponse.failure(
                            result.getError().getCode(),
        ...
        if (result.getValue().isEmpty()) {
            log.warn("REST: User not found with userName: {}", userName);
            return ResponseEntity
                    .status(HttpStatus.NOT_FOUND)
                    .body(ApiResultResponse.failure(
                            "user.not.found",
        ...
        log.info("REST: Successfully fetched user with userName: {}", userName);
        return ResponseEntity.ok(
                ApiResultResponse.success(result.getValue().get())
        );
    }
    ...
    private HttpStatus determineHttpStatus(String errorCode) {
        return switch (errorCode) {
            case "entity.not.found", "record.not.found", "user.not.found" -> HttpStatus.NOT_FOUND;
